...
def build_trainer(opt, model, fields, optim, data_type, logger,
    ...
    trainer = Trainer(model, train_loss, valid_loss, optim, trunc_size,
                           ...
                           logger, model_saver=model_saver, rl_gamma=rl_gamma)
...
class Trainer(object):
                        ...
                        if self.gpu_verbose_level > 0:
                            logger.info("GpuRank %d: reduce_counter: %d \
                                        ...
                                        % (self.gpu_rank, reduce_counter,
                                           len(true_batchs)))
                        if self.n_gpu > 1:
                            normalization = sum(utils.distributed
                                                .all_gather_list
                                                (normalization))
    ...
    def _gradient_accumulation(self, true_batchs, normalization, total_stats,
        ...
        if self.n_gpu > 1:
            grads = [p.grad.data for p in self.model.parameters()
                     if p.requires_grad]
            utils.distributed.all_reduce_and_rescale_tensors(
                grads, float(1))
    ...
    def _maybe_gather_stats(self, stat):
        ...
        if stat is not None and self.n_gpu > 1:
            return utils.Statistics.all_gather_stats(stat)
        return stat
