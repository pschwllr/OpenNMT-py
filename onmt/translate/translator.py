...
def build_translator(opt, report_score=True, logger=None, out_file=None, probs_out_file=None):
    ...
    kwargs = {k: getattr(opt, k)
              for k in ["beam_size", "n_best", "max_length", "min_length",
                        ...
                        "data_type", "replace_unk", "gpu", "verbose", "fast", "mask_from"]}
...
class Translator(object):
    ...
    def __init__(self,
        ...
        if mask_from != '':
            from ..utils.masking import ChemVocabMasker
            self.masker = ChemVocabMasker(from_file=mask_from)
        else:
            self.masker = None
    ...
    def translate_single(self, src_tokens, tgt_tokens=None, fast=False):
        """
        ...
        with torch.no_grad():
            if fast:
                return self._fast_translate_single(src_tokens, tgt_tokens)
            else:
                return self._translate_single(src_tokens, tgt_tokens)


    def _translate_single(self, src_tokens, tgt_tokens=None):
        """
        Translate a batch of sentences.
        ...
        src = torch.LongTensor([src_vocab.stoi[token] for token in src_tokens])
        src = src.unsqueeze(1).unsqueeze(2)
        src_lengths = torch.LongTensor([src.size()[0]])
        ...
        if tgt_tokens is not None:
            ret["gold_score"] = self._run_target_single(src, tgt_tokens)
        # ret["batch"] = batch

        predicted_tokens = []
        for prediction in ret['predictions'][0]:
            tokens = []
    ...
    def _fast_translate_single(self, src_tokens, tgt_tokens=None):
        ...
        results['context_attns'] = dec_states.context_attn.data if dec_states.context_attn is not None else None
    ...
    def _fast_translate_batch(self,
        ...
        for step in range(max_length):
            ...
            if self.masker is not None:
                ...
                # Multiply probs by the beam probability.
            log_probs = log_probs + topk_log_probs.view(-1).unsqueeze(1)
    ...
    def _fast_sample_batch(self,
        ...
        for step in range(max_length):
            ...
            # Multiply probs by the beam probability.
            log_probs = log_probs + sampled_log_probs.view(-1).unsqueeze(1)
            ...
            distribution = Categorical(logits=curr_scores)

            sampled_ids = distribution.sample().unsqueeze(1)
            sampled_scores = curr_scores.gather(1, sampled_ids)

            # topk_scores, topk_ids = curr_scores.topk(beam_size, dim=-1)

            # Recover log probs.
            sampled_log_probs = sampled_scores * length_penalty

            # Resolve beam origin and true word ids.
            sampled_beam_index = sampled_ids.div(vocab_size)
            sampled_ids = sampled_ids.fmod(vocab_size)

            # Map beam_index to batch_index in the flat representation.
            batch_index = (
                sampled_beam_index
                + beam_offset[:sampled_beam_index.size(0)].unsqueeze(1))

            # End condition is the top beam reached end_token.
            end_condition = sampled_ids[:, 0].eq(end_token)
            if step + 1 == max_length:
            ...
            if len(finished) > 0:
                # set_trace()
                predictions = alive_seq.view(-1, beam_size, alive_seq.size(-1))
                scores = sampled_scores.view(-1, beam_size)
                attention = None
                if alive_attn is not None:
                ...
                if len(non_finished) == 0:
                    break
                # Remove finished batches for the next step.
                sampled_log_probs = sampled_log_probs.index_select(
                    0, non_finished.to(sampled_log_probs.device))
                sampled_ids = sampled_ids.index_select(0, non_finished)
                batch_index = batch_index.index_select(0, non_finished)
                batch_offset = batch_offset.index_select(0, non_finished)
            ...
            # Append last prediction.
            alive_seq = torch.cat([alive_seq, sampled_ids.view(-1, 1)], -1)
    ...
    def _run_target(self, batch, data):
        ...
        for dec, tgt in zip(dec_out, batch.tgt[1:].data):
            ...
            gold_scores = gold_scores + scores.view(-1)
        return gold_scores


    def _run_likelihood(self, batch, sequences):
        """
        Calculates the log likelihood of a given sequence.
        Predictions should have the shape of [max_length, batch_size]
        and start with the start_token. Same as a batch.tgt
        :param batch:
        :param data:
        :param prediction:
        :return:
        ...
        tgt_pad = self.fields["tgt"].vocab.stoi[inputters.PAD_WORD]
        for dec, tgt in zip(dec_out, sequences[1:].data):
            # Log prob of each word.
            out = self.model.generator.forward(dec)
            tgt = tgt.unsqueeze(1)
            scores = out.data.gather(1, tgt)
    ...
    def _run_likelihood_single(self, src_tokens, target_tokens):
        vocab = self.fields["tgt"].vocab
        src_vocab = self.fields["src"].vocab
        src = torch.LongTensor([src_vocab.stoi[token] for token in src_tokens])
        src = src.unsqueeze(1).unsqueeze(2)
        ...
        target = torch.LongTensor([vocab.stoi[token] for token in [inputters.BOS_WORD] + target_tokens])

        tgt_in = target.unsqueeze(1).unsqueeze(2)

        #  (2) if a target is specified, compute the 'goldScore'
        #  (i.e. log likelihood) of the target under the model
        tt = torch.cuda if self.cuda else torch
        gold_scores = tt.FloatTensor(batch_size).fill_(0)
        dec_out, _, _ = self.model.decoder(
            tgt_in, memory_bank, dec_states, memory_lengths=src_lengths)

        tgt_pad = self.fields["tgt"].vocab.stoi[inputters.PAD_WORD]
        for dec, tgt in zip(dec_out, target.unsqueeze(1)[1:].data):
            # Log prob of each word.
            out = self.model.generator.forward(dec)
            tgt = tgt.unsqueeze(1)
            scores = out.data.gather(1, tgt)
